...
def run_multistream(M,output_dir="./"):
    ...
    # Output setup - NOW USING OUTPUT_DIR
    step = 0
    output_freq = 20
    outfile = VTKFile(os.path.join(output_dir, f"multistream_M{M}_vlasov.pvd"))
    ...
    # Initial checkpoint - NOW USING OUTPUT_DIR
    with CheckpointFile(os.path.join(output_dir, f"multistream_M{M}_initial_checkpoint.h5"),'w') as afile:
    ...
    with CheckpointFile(os.path.join(output_dir, f"multistream_M{M}_final_checkpoint.h5"),'w') as afile:
        afile.save_function(phi, name="phi")
        afile.save_mesh(mesh, "1d_mesh")
...
if __name__ == "__main__":
    ...
    for M in [2, 4, 8]:
        ...
        run_multistream(M, output_dir=f"results/M_{M}")